    ...
    <div className={cn('rounded-lg border border-white/10 bg-slate-900/60 p-4', item.severity === 'critical' && answer !== 'yes' ? 'ring-1 ring-rose-400/30' : '')}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          ...
          <p className="text-sm font-semibold text-white">{getLocalizedTechnicalText(item.title, lang)}</p>
          <p className="text-sm leading-6 text-slate-300">{getLocalizedTechnicalText(item.question, lang)}</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-md border border-emerald-400/15 bg-emerald-400/10 p-3">
          ...
          <p className="mt-2 text-sm leading-6 text-emerald-50">{getLocalizedTechnicalText(item.successSignal, lang)}</p>
        </div>
        <div className="rounded-md border border-rose-400/15 bg-rose-400/10 p-3">
          ...
          <p className="mt-2 text-sm leading-6 text-rose-50">{getLocalizedTechnicalText(item.quickFix, lang)}</p>
        </div>
      </div>
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        <div className="rounded-md border border-white/10 bg-slate-950/50 p-3">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">{getLocalizedTechnicalText(item.failureSignal, lang)}</p>
        </div>
