...
const MicroDetailCardInner = ({ detail, lang }: Props) => {
  ...
  return (
    ...
    <p className="mt-1 text-sm leading-6 text-slate-400">{getLocalizedTechnicalText(detail.whyItWorks, lang)}</p>
    <p className="mt-3 rounded-md border border-emerald-300/15 bg-emerald-300/10 px-3 py-2 text-sm text-emerald-100">{getLocalizedTechnicalText(detail.correctionCue, lang)}</p>
