    ...
    <div className="mt-4 space-y-3">
      {path.steps.slice(0, 6).map((step) => (
        <div key={step.id} className="rounded-md border border-white/10 bg-slate-950/60 p-3">
          ...
          <p className="mt-2 text-xs text-slate-500">{getLocalizedText(step.commonMistake, lang)}</p>
        </div>
      ))}
    </div>
    <p className="mt-4 text-sm leading-6 text-emerald-50"><span className="font-semibold">{t('microDetailSystem.trigger')}: </span>{getLocalizedText(path.finishTrigger, lang)}</p>
    <p className="mt-2 text-sm leading-6 text-slate-300"><span className="font-semibold text-white">{t('microDetailSystem.abort')}: </span>{getLocalizedText(path.abortSignal, lang)}</p>
    <p className="mt-2 text-sm leading-6 text-slate-300"><span className="font-semibold text-white">{t('microDetailSystem.next')}: </span>{getLocalizedText(path.nextBestOption, lang)}</p>
