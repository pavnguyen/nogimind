...
const makeChecklist = (id: string, title: string, kind: 'control' | 'escape' | 'guard' | 'wrestling' | 'safety', safety?: boolean): TechniqueQualityChecklist => {
  ...
  const checks: QualityCheckItem[] = [
    ...
    mkCheck(
      `${id}-4`, 
      kind === 'guard' ? lt('Lớp guard', 'Guard layer', 'Couche de garde') : kind === 'escape' ? lt('Đường trong', 'Inside position', 'Inside position') : kind === 'wrestling' ? lt('Off-balance hoặc shot', 'Off-balance or shot', 'Off-balance ou shot') : lt('Thân dưới', 'Lower body', 'Bas du corps'), 
