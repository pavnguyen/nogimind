...
const makeSystem = (id: string, title: string, overview: string, kind: 'control' | 'escape' | 'guard' | 'wrestling' | 'safety'): MicroDetailSystem => {
  ...
  return {
    ...
    topFiveDetails: [
      ...
      mkDetail(`${id}-branch`, 'timing', `${title} branch`, `If blocked, change lane early.`, `When the first line is denied`, `Branching keeps the system moving.`, `You force one lane too long.`, 'Branch early.', 'Branch early.', 'both', 'open_out', ['hands', 'hips', 'head']),
    ],
    leftRightGuides: [
    ...
    fastFinishPaths: [mkPath(
      ...
      [
        ...
        [kind === 'escape' ? 'Branch to guard or turtle.' : kind === 'guard' ? 'Recover the next layer.' : kind === 'wrestling' ? 'Finish or re-attack.' : kind === 'safety' ? 'Stop if safety is unclear.' : 'Branch if blocked.', 'feet', 'You freeze on the first option.'],
      ],
