...
const progressions = (safety = false): TechniqueStateMachine['trainingProgressions'] => [
  ...
  {
    phase: safety ? lt('Xem xét an toàn', 'Safety review', 'Revue sécurité') : lt('Round vị trí', 'Positional round', 'Round positionnel'),
...
export const techniqueStateMachines: TechniqueStateMachine[] = [
  ...
  machine({
    ...
    outcomes: [
      outcome('at-success', 'success', lt('Kết thúc arm triangle', 'Arm triangle finish', 'Finish arm triangle'), lt('Shoulder của họ ép vào cổ, shoulder pressure đi chéo và hips đã tạo angle.', 'Their shoulder compresses the neck, shoulder pressure runs diagonally, and hips create angle.', 'Son épaule compresse cou, pression épaule diagonale, hanches créent angle.'), lt('Elbow bị kẹt qua centerline và họ không đưa tay về ribs.', 'Elbow is trapped across centerline and cannot return to ribs.', 'Coude piégé au-delà centerline.'), { toSubmissionId: 'arm-triangle-mount' }, 'high'),
...
const extraSpecs: Spec[] = [
  {
    ...
    outcomes: [
      outcome('guillotine-success', 'success', lt('Guillotine sạch', 'Clean guillotine', 'Guillotine propre'), lt('Chin strap sâu, elbow đóng và hip angle làm cổ bị đóng line.', 'Deep chin strap, closed elbow, and hip angle close the neck line.', 'Chin strap profond, coude fermé, angle hanche ferme la ligne.'), lt('Đầu họ thấp hơn shoulder line.', 'Their head is below shoulder line.', 'Tête sous shoulder line.'), { toSubmissionId: 'guillotine-system' }, 'high'),
  ...
  },
  {
    ...
    outcomes: [
      ...
      outcome('kimura-safety', 'safety_abort', lt('Hủy shoulder torque', 'Abort shoulder torque', 'Stop torque épaule'), lt('Không torque nhanh khi shoulder line chưa được kiểm soát hoặc partner tap.', 'Do not torque fast when shoulder line is uncontrolled or partner taps.', 'Ne pas torque vite si shoulder line pas contrôlée ou tap.'), lt('Partner tap hoặc shoulder xoắn bất thường.', 'Partner taps or shoulder twists unusually.', 'Partner tape ou épaule tourne anormalement.'), { toSafetyNoteId: 'shoulder_lock_safety' }, 'high'),
    ],
  },
  {
    ...
    outcomes: [
      outcome('heel-safe', 'success', lt('Knee line sạch', 'Knee line free', 'Knee line libre'), lt('Defender clear knee line trước rotation nên leg lock danger giảm.', 'Defender clears knee line before rotation, reducing leg-lock danger.', 'Défenseur clear knee line avant rotation, danger réduit.'), lt('Knee vượt hip line và heel không bị exposed.', 'Knee passes hip line and heel is not exposed.', 'Genou passe hip line et talon non exposé.'), { toSkillId: 'leg-lock-safety-basics' }, 'high'),
      ...
      outcome('heel-abort', 'safety_abort', lt('Hủy rotation', 'Abort rotation', 'Stop rotation'), lt('Nếu heel exposed và knee trapped, tap/reset thay vì xoay.', 'If heel is exposed and knee is trapped, tap or reset instead of turning.', 'Si talon exposé et genou piégé, taper/reset au lieu de tourner.'), lt('Heel bị bắt và knee chưa clear.', 'Heel is caught and knee is not clear.', 'Talon pris et genou pas clear.'), { toSafetyNoteId: 'heel_hook_safety' }, 'high'),
    ],
  },
]
...
;[
  ...
  ['x-guard-control', 'x-guard-position', 'X-Guard uses foot on hip and leg wrap to off-balance and sweep.', 'X-Guard dùng chân trên hông và wrap chân để off-balance và sweep.', 'X-Guard utilise pied sur hanche et wrap jambe pour déséquilibrer et balayer.', 'single-leg-x-control'],
  ['single-leg-x-control', 'x-guard-position', 'Single Leg X controls one leg with both legs for sweeps and entries.', 'Single Leg X kiểm soát một chân bằng hai chân để sweep và entries.', 'Single Leg X contrôle une jambe avec deux jambes pour balayages et entrées.', 'k-guard-entry'],
  ['closed-guard-posture-control', 'closed-guard', 'Posture control breaks their alignment to open sweeps and submissions.', 'Kiểm soát posture phá alignment để mở sweeps và submissions.', 'Contrôle posture casse alignement pour ouvrir sweeps et soumissions.', 'closed-guard-sweeps'],
  ['closed-guard-sweeps', 'closed-guard', 'Hip bump, scissor, and flower sweeps off-balance and reverse the position.', 'Hip bump, scissor, flower sweeps làm mất thăng bằng và đảo position.', 'Hip bump, scissor, flower sweeps déséquilibrent et inversent la position.', 'side-control-pin'],
  ['de-la-riva-sweeps', 'de-la-riva-guard', 'DLR hook and sleeve control create sweep angles into X-guard or top position.', 'DLR hook và sleeve control tạo sweep angle vào X-guard hoặc top position.', 'DLR hook et sleeve control créent angles de sweep vers X-guard ou top.', 'x-guard-control'],
  ['de-la-riva-back-take', 'de-la-riva-guard', 'Berimbolo spin uses the DLR hook and far hip control to expose the back.', 'Berimbolo spin dùng DLR hook và far hip control để mở lưng.', 'Berimbolo spin utilise DLR hook et far hip pour exposer le dos.', 'back-control'],
  ['reverse-de-la-riva-transitions', 'reverse-de-la-riva-guard', 'Outside hook and hip rotation transition to DLR, X-guard, or kimura.', 'Outside hook và hip rotation chuyển sang DLR, X-guard hoặc kimura.', 'Outside hook et rotation hanche passent en DLR, X-guard ou kimura.', 'de-la-riva-sweeps'],
  ['z-guard-sweeps', 'z-guard', 'Deep half and waiter sweeps off-balance the passer to open the underhook path.', 'Deep half và waiter sweeps làm passer mất thăng bằng để mở underhook lên top.', 'Deep half et waiter sweeps déséquilibrent le passeur pour ouvrir underhook.', 'side-control-pin'],
  ['knee-on-belly-control', 'knee-on-belly', 'Knee pressure isolates the arm and creates transitions to mount or back take.', 'Áp lực gối cô lập tay và tạo transition lên mount hoặc back take.', 'Pression genou isole le bras et crée transitions vers mount ou back take.', 'mount-control'],
  ['north-south-control', 'north-south', 'Shoulder pressure and kimura grip control open transitions to back take or choke.', 'Shoulder pressure và kimura grip mở transition lên back take hoặc choke.', 'Pression épaule et kimura grip ouvrent transitions vers back take ou choke.', 'rear-naked-choke-system'],
  ['inverted-guard-control', 'inverted-guard', 'Invert into K-guard or matrix entries for leg attacks and sweeps.', 'Invert vào K-guard hoặc matrix entries cho leg attacks và sweeps.', 'Inverser vers K-guard ou matrix entries pour attaques jambes et sweeps.', 'k-guard-matrix'],
  ...
  extraSpecs.push({
    ...
    outcomes: [
      ...
      ...(['front-headlock-defense', 'false-reap-entry', 's-mount-armbar', 'buggy-choke', 'gogoplata', 'choi-bar', 'tarikoplata'].includes(skillId)
        ? [outcome(`${skillId}-safety`, 'safety_abort', lt('Hủy vì an toàn', 'Safety abort', 'Stop sécurité'), lt('Nếu có đau cổ, vai, gối hoặc tap, release ngay và reset.', 'If neck, shoulder, knee pain or tap appears, release immediately and reset.', 'Si douleur cou/épaule/genou ou tap, relâcher immédiatement.'), lt('Pain tăng nhanh hoặc partner tap.', 'Pain rises fast or partner taps.', 'Douleur augmente vite ou partenaire tape.'), { toSafetyNoteId: `${skillId}-safety` }, 'high')]
        : []),
    ],
  })
})
