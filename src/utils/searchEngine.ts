...
export const setSearchData = (bundle: SearchDataBundle) => {
  data = bundle
}

const getData = (): SearchDataBundle => {
  if (!data) throw new Error('Search data not initialized. Call setSearchData() first.')
  return data
}
...
const skillDocument = (skill: SkillNode, lang: LanguageCode): SearchDocument => {
  ...
  return {
    ...
    fields: [
      ...
      field('details', [skill.microDetailSystem, skill.blackbeltDetails, skill.qualityChecklist, skill.commonMistakes], lang, 6),
...
const microDetailDocuments = (lang: LanguageCode): SearchDocument[] =>
  getData().microDetails.map((detail) => ({
...
/** Sync version of searchKnowledge for Node.js scripts and worker use */
